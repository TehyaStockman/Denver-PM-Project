met_processing <- function(raw_data){
  ...
  if("PM2.5...Local.Conditions" %in% colnames(raw_data) & "Nitric.oxide" %in% colnames(raw_data) & "Carbon.monoxide" %in% colnames(raw_data)){
    names(raw_data)[names(raw_data) == "PM2.5...Local.Conditions"] <- "pm25_p.val"
    names(raw_data)[names(raw_data) == "Carbon.monoxide"] <- "CO.val"
    names(raw_data)[names(raw_data) == "Nitrogen.dioxide"] <- "NO2.val"
    names(raw_data)[names(raw_data) == "Nitric.oxide"] <- "NO.val"
    keeps <- c("Date", "Outdoor.Temperature", "Wind.Direction", "Wind.Speed", "pm25_p.val", "CO.val", "NO2.val", "NO.val")
    
  }else if (!("PM2.5...Local.Conditions" %in% colnames(raw_data)) & "Nitric.oxide" %in% colnames(raw_data) 
            & "Carbon.monoxide" %in% colnames(raw_data) & "Outdoor.Temperature" %in% colnames(raw_data)){
              
    names(raw_data)[names(raw_data) == "Carbon.monoxide"] <- "CO.val"
    names(raw_data)[names(raw_data) == "Nitrogen.dioxide"] <- "NO2.val"
    names(raw_data)[names(raw_data) == "Nitric.oxide"] <- "NO.val"
    keeps <- c("Date", "Outdoor.Temperature", "Wind.Direction", "Wind.Speed", "CO.val", "NO2.val", "NO.val")
  ...
  }else if ("PM2.5...Local.Conditions" %in% colnames(raw_data) & ("Nitrogen.dioxide" %in% colnames(raw_data))){
    names(raw_data)[names(raw_data) == "PM2.5...Local.Conditions"] <- "pm25_p.val"
    names(raw_data)[names(raw_data) == "Nitrogen.dioxide"] <- "NO2.val"
    keeps <- c("Date", "Wind.Direction", "Wind.Speed", "pm25_p.val", "NO2.val")
    
  }else if ("PM2.5...Local.Conditions" %in% colnames(raw_data) & !("Outdoor.Temperature" %in% colnames(raw_data) & !("Nitrogen.dioxide" %in% colnames(raw_data)))){
    names(raw_data)[names(raw_data) == "PM2.5...Local.Conditions"] <- "pm25_p.val"
    keeps <- c("Date", "Wind.Direction", "Wind.Speed", "pm25_p.val")
    
  }else if (!("PM2.5...Local.Conditions" %in% colnames(raw_data)) & !("Outdoor.Temperature" %in% colnames(raw_data)) 
             !("Carbon.monoxide" %in% colnames(raw_data))  & !("Nitrogen.dioxide" %in% colnames(raw_data))){
    ...
    names(raw_data)[names(raw_data) == "Carbon.monoxide"] <- "CO.val"
    names(raw_data)[names(raw_data) == "Nitrogen.dioxide"] <- "NO2.val"
    keeps <- c("Date", "Wind.Direction", "Wind.Speed", "CO.val", "NO2.val")
